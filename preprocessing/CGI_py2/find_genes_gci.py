...
def _find_genes_gci_pc(data: np.ndarray, alpha: float = 0.05) -> dict:
    # 默认最后一列是目标变量
    n = data.shape[1] - 1
    x = data[:, -1].copy()
    data = data[:, :n]
...
def _ci_independent(full_vars_norm: np.ndarray, idx_a: int, idx_b: int, cond_idxs: list, alpha: float) -> bool:
    """使用 paco_test 判断条件独立，True 表示独立。"""
...
def _run_pc_with_target(full_vars: np.ndarray, target_idx: int, alpha: float) -> set:
    """
    在 full_vars 中将 target_idx 作为目标，运行 PC 发现并映射回原索引。
    注意：调用的是 _find_genes_gci_pc（单层 PC），避免递归到 MB 封装。
    """
    ...
    res = _find_genes_gci_pc(new_data, alpha=alpha)
    neighbors = set()
...
def find_genes_gci(data: np.ndarray, alpha: float = 0.05) -> dict:
    """
    单层 Markov Blanket 发现：
    - 先得到 PC(T)（候选父/子）
    - 再通过二阶邻接 + CI 证据识别 spouse
    - 尽量判定 parent/child，未定向并入 parent
    - found_genes 输出 MB 全集
    """
    n_genes = data.shape[1] - 1
    t_idx = n_genes

    # 第1步：PC(T)
    res_pc = _find_genes_gci_pc(data, alpha=alpha)
    pc_t = sorted([g for g in res_pc['found_genes'] if 0 <= g < n_genes])
    pc_t_set = set(pc_t)
    ...
    for x in pc_t:
        neighbors_x = _run_pc_with_target(full_vars, x, alpha=alpha)
    ...
    found_genes = sorted(parents | children | spouses)
    print_matlab_style_array('found_Genes', found_genes)

    return {
        'found_genes': found_genes,
        'markov_blanket': found_genes,
